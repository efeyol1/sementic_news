  ...
  return (
    <div className="space-y-2">
        ...
        return (
          <div key={name} className="flex items-center gap-3">
            ...
            <div className="flex-1 h-5 rounded-md overflow-hidden flex gap-0.5 bg-slate-100">
              <div
                className="h-full bg-green-400 flex items-center justify-center text-xs font-semibold text-white rounded-l-md"
                style={{ width: `${pos}%`, minWidth: pos > 0 ? 4 : 0 }}
              >
                {pos > 15 && `${pos.toFixed(0)}%`}
              </div>
              <div
                className="h-full bg-red-400 flex items-center justify-center text-xs font-semibold text-white rounded-r-md"
      ...
      <div className="flex items-center gap-4 pt-3 border-t border-slate-100">
        {[{ color: "bg-green-400", label: "Pozitif" }, { color: "bg-red-400", label: "Negatif" }].map(({ color, label }) => (
